...
public class Mg18parrerl : MonoBehaviour
{
    ...
    private void CalculateMoveSpeedByLayer(GameObject[] backgrounds, int count)
    {
        ...
        for (int i = 0; i < count; ++i)
        {
            ...
            layerMoveSpeed[i] = 10 - (backgrounds[i].transform.position.z - cameraTransform.position.z) / farthestBackDistance;
            // 이동 속도 확인용 (테스트 후 삭제)
            // Debug.Log($"{layerMoveSpeed[i]}, 실제 이동 속도 = {layerMoveSpeed[i] * parallaxSpeed}");
        }
    }
